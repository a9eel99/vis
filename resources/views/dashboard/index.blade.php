...
<div class="dash-kpis">
    <div class="card dash-kpi" style="border-{{ $isRtl ? 'right' : 'left' }}:4px solid var(--primary)">
        ...
        <div class="dash-kpi-value" style="color:var(--primary)">{{ $stats['total_inspections'] }}</div>
        <div class="dash-kpi-sub">{{ $isRtl ? 'هذا الشهر:' : 'This month:' }} <span style="color:var(--primary);font-weight:700">{{ $stats['this_month'] }}</span></div>
    </div>
    <div class="card dash-kpi" style="border-{{ $isRtl ? 'right' : 'left' }}:4px solid #10b981">
        ...
        <div class="dash-kpi-value" style="color:#10b981">{{ $stats['pass_rate'] }}%</div>
        <div class="dash-kpi-sub">{{ $isRtl ? 'نجح:' : 'Passed:' }} {{ $stats['passed'] }} &bull; {{ $isRtl ? 'فشل:' : 'Failed:' }} {{ $stats['failed'] }}</div>
    </div>
    <div class="card dash-kpi" style="border-{{ $isRtl ? 'right' : 'left' }}:4px solid #3b82f6">
        ...
        <div class="dash-kpi-value" style="color:#3b82f6">{{ $stats['average_score'] }}%</div>
    ...
    <div class="card dash-kpi" style="border-{{ $isRtl ? 'right' : 'left' }}:4px solid #f59e0b">
        ...
        <div class="dash-kpi-value" style="color:#f59e0b">{{ $todayCount }}</div>
        <div class="dash-kpi-sub">{{ $isRtl ? 'مكتمل:' : 'Completed:' }} <span style="color:#10b981;font-weight:700">{{ $todayCompleted }}</span></div>
    </div>
</div>
...
<div class="dash-charts">
    <div class="card">
        <div class="card-header"><h3>📈 {{ $isRtl ? 'الفحوصات الشهرية' : 'Monthly Inspections' }}</h3></div>
        <div class="card-body dash-chart-box"><canvas id="monthlyChart"></canvas></div>
    </div>
    <div class="card">
        <div class="card-header"><h3>📊 {{ $isRtl ? 'توزيع التقييمات' : 'Grade Distribution' }}</h3></div>
        <div class="card-body dash-chart-box" style="display:flex;align-items:center;justify-content:center">
            ...
            @else
            <p style="color:var(--gray-400);font-size:.9rem">{{ $isRtl ? 'لا توجد بيانات بعد' : 'No data yet' }}</p>
            @endif
        </div>
    </div>
</div>
...
<div class="card mb-2">
    <div class="card-header"><h3>📉 {{ $isRtl ? 'متوسط النتيجة الشهري' : 'Monthly Avg Score' }}</h3></div>
...
<div class="dash-bottom">
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr style="cursor:pointer" onclick="window.location='{{ route('inspections.show', $ins) }}'">
                        <td>
                            <div style="font-weight:600;font-size:.85rem">{{ $ins->vehicle?->make }} {{ $ins->vehicle?->model }}</div>
                            <div style="font-size:.72rem;color:var(--gray-400)">{{ $ins->vehicle?->license_plate }}</div>
                        </td>
                        ...
                        <td style="font-size:.78rem;color:var(--gray-500);white-space:nowrap">{{ $ins->created_at->diffForHumans() }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            @else
            <div class="empty-state"><div class="empty-state-icon">📋</div><p>{{ __('no_inspections') }}</p></div>
            @endif
        </div>
    </div>
    ...
    <div>
        ...
        <div class="card mb-2">
            <div class="card-header"><h3>⚡ {{ __('quick_actions') }}</h3></div>
            <div class="card-body">
                <div class="quick-actions">
                    @can('create inspections')
                    <a href="{{ route('inspections.create') }}" class="quick-action-btn primary-action">{{ __('new_inspection') }}</a>
                    @endcan
                    @can('create vehicles')
                    <a href="{{ route('vehicles.create') }}" class="quick-action-btn">🚗 {{ __('add_vehicle') }}</a>
                    @endcan
                    @can('view audit logs')
                    <a href="{{ route('audit-logs.index') }}" class="quick-action-btn">📊 {{ __('reports') }}</a>
                    @endcan
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header"><h3>👥 {{ __('inspectors') }}</h3></div>
            <div class="card-body" style="padding:0">
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;border-bottom:1px solid var(--gray-100)">
                    <div style="width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:.85rem;flex-shrink:0;background:{{ ['#dbeafe','#dcfce7','#fef3c7','#fee2e2','#f3e8ff'][$loop->index % 5] }};color:{{ ['#1e40af','#166534','#92400e','#991b1b','#6b21a8'][$loop->index % 5] }}">
                        {{ mb_substr($inspector->name, 0, 1) }}
                    </div>
                    <div style="flex:1;min-width:0">
                        <div style="font-weight:600;font-size:.85rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $inspector->name }}</div>
                        <div style="font-size:.72rem;color:var(--gray-400)">{{ $inspector->completed_count }} {{ $isRtl ? 'مكتمل' : 'completed' }}</div>
                    </div>
                    <div style="text-align:center;flex-shrink:0">
                        <div style="font-size:1.1rem;font-weight:800;color:var(--primary)">{{ $inspector->inspections_count }}</div>
                        <div style="font-size:.65rem;color:var(--gray-400)">{{ $isRtl ? 'فحص' : 'total' }}</div>
                    </div>
                </div>
                @empty
                <p class="text-muted text-center" style="padding:1rem;font-size:.9rem">{{ __('no_inspectors') }}</p>
                @endforelse
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (mCtx) {
        new Chart(mCtx, {
            ...
            data: {
                labels: @json($mLabels),
                datasets: [
                    { label: '{{ $isRtl ? "إجمالي" : "Total" }}', data: @json($mTotals), backgroundColor: 'rgba(30,58,95,0.15)', borderColor: '#1e3a5f', borderWidth: 1.5, borderRadius: 4 },
                    { label: '{{ $isRtl ? "مكتمل" : "Completed" }}', data: @json($mCompleted), backgroundColor: 'rgba(16,185,129,0.2)', borderColor: '#10b981', borderWidth: 1.5, borderRadius: 4 }
                ]
            },
    ...
    if (gCtx) {
        new Chart(gCtx, {
            ...
            data: {
                ...
                datasets: [{ data: [{{ $gradeDist['excellent'] ?? 0 }},{{ $gradeDist['good'] ?? 0 }},{{ $gradeDist['needs_attention'] ?? 0 }},{{ $gradeDist['critical'] ?? 0 }}], backgroundColor: ['#10b981','#3b82f6','#f59e0b','#ef4444'], borderWidth: 2, borderColor: '#fff' }]
            },
    ...
    if (sCtx) {
        new Chart(sCtx, {
            ...
            data: {
                labels: @json($mLabels),
                datasets: [{ label: '{{ $isRtl ? "متوسط النتيجة %" : "Avg Score %" }}', data: @json($mAvgScores), borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.08)', borderWidth: 2.5, pointRadius: 5, pointBackgroundColor: '#3b82f6', tension: 0.3, fill: true }]
            },
